...
const Add = () => {
  ...
  const handleAddProduct = async(e)=>{
    ...
    try {
      ...
      let result  = await axios.post (serverUrl+"/api/product/addProduct",formData,formData,{withCredentials:true})
      ...
      <div className="w-[82%] h-[100%] flex items-center justify-start overflow-x-hidden absolute right-0 bottom-[5%]">
        <form action="" onSubmit={handleAddProduct} className='w-[100%] md:w-[90%] h-[100%] mt-[70px] flex flex-col gap-[30px] py-[60px] px-[30px] md:px-[60px]'>
          ...
          <div className='w-[80%]  flex items-start justify-center flex-col gap-[10px] '>
            ...
            <textarea type="text" placeholder='Type here' className='w-[600px] max-w-[98%] h-[100px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer bg-slate-600 px-[20px] py-[10px] text-[18px] placeholder:text-[#ffffffc2]' value={description} onChange={(e)=>setDescription(e.target.value)}  required/>
          </div>
          <div className='w-[80%] flex items-center gap-[10px] flex-wrap'>
            <div className='md:w-[40%] w-[100%] flex items-start sm:justify-center flex-col gap-[10px]'>
              <p className='text-[20px] md:text-[25px] font-semibold w-[100%]'>Product Category</p>
              <select name="" id="" className='bg-slate-600 w-[60%] px-[10px] py-[7px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer' value={category} onChange={(e)=>setCategory(e.target.value)} >
            ...
            <div className='md:w-[40%] w-[100%] flex items-start sm:justify-center flex-col gap-[10px] ' >
              ...
              <select name="" id="" className='bg-slate-600 w-[60%] px-[10px] py-[7px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer' value={subCategory} onChange={(e)=>setSubCategory(e.target.value)}>
          ...
          <div className='w-[80%] h-[100px] flex items-start justify-center flex-col gap-[10px] '>
            ...
            <input type="number" placeholder='₹ 5000' className='w-[600px] max-w-[98%] h-[40px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer bg-slate-600 px-[20px] text-[18px] placeholder:text-[#ffffffc2]' min={0} value={price} onChange={(e)=>setPrice(e.target.value)} required/>
          </div>
          ...
          <div className='w-[80%] flex items-center justify-start gap-[10px] mt-[20px]'>
            ...
            <input type="checkbox" id='checkbox' className='w-[25px] h-[25px] cursor-pointer ' onChange={()=>setBestSeller(prev=>!prev)} value={bestseller} />
            <label htmlFor="checkbox" className='text-[18px] md:text-[22px] font-semibold'>Add to BestSeller</label>
          </div>

          <button className='w-[140px] px-[20px] py-[20px] bg-[#65d8f7] hover:bg-cyan-500 flex items-center justify-center gap-[10px] text-black active:bg-slate-700 active:text-white active:border-[2px] border-white rounded-xl cursor-pointer'>{loading ? <Loading/> : "Add Product"}</button>
